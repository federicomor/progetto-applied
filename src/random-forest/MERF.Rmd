...
ME_formula_str <- paste("Social.well.being~fix_effect.pred", paste(random_intercepts,collapse="+"), sep="+")
```
...
# sigma2_f <- as.numeric(get_variance_fixed(lme_fit)) 
# -> this is what is computed by the statistic r.squaredGLMM
...
mean((pisa_data$Social.well.being-predict(lme_fit))^2)
```
...
fit_lmerf$forest.model$mse[length(fit_lmerf$forest.model$mse)]
sum((y-y_hat)^2)/length(y_hat)
```
...
sigma2_f <- var(fit_lmerf$forest.model$predicted)

pseudo_Rsq <- (sigma2_b+sigma2_f)/(sigma2_b+sigma2_eps+sigma2_f)
pseudo_Rsq
```
