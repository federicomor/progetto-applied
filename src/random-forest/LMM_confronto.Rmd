...
```{r}
for (state in STATES)
	print(paste("#",state,"=",sum(dati_soc_woo==state)))
```
...
```{r}
for (state in STATES)
	print(paste("#",state,"=",sum(dati_psi_woo==state)))
```
...
Ufficiali per ora. Sono gli stessi di sopra finché le soglie non vengono cambiate da 24.3 e 24.26.
...
Check da spuntare: sono i seguenti file riferiti ai dati della versione-2? - social sì (c'era il codice già pronto) - psych sì anche lui fatto (ma Giulia se vuoi ricontrollare è meglio, perché ho solo copiato e incollato il codice di prima, senza fare tuning di eventuali parametri (ma mi sembrava tutto automatico, cioè guidato da variabili anziché scelte da fare a mano))
