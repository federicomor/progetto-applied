# Simple LM with CNT as covariate
...
to_discard <- c("Social.well.being", "Psychological.well.being", "SCHLTYPE","PV1MATH", "SchoolUse.ICT")
...
num_var_SWB <- which.max(reg.summarySWB$adjr2)
num_var_SWB 
...
selected_vars_SWB<- c(fullSWB[2:13],"CNT")
...
threshold_lev <- 0.04 #rule of thumb 2*p/n = 0.01102435 (seems to low frmo the plot)
...
threshold_cook_dist <- 0.008 #rule of thumb 4/(n-p)=0.0009237875 seems too low
...
```{r}
suppressWarnings({fit_data = lm(FORMULA_SOCIAL,data=data)})
res = residuals(fit_data)
qqnorm(res,main="data", ylim=c(-6,6))
qqline(res,col="red")

suppressWarnings({fit_data_soc_woo = lm(FORMULA_SOCIAL,data=dati_soc_woo)})
...
```{r}
df <- dati_soc_woo
res <- residuals.lm(fit_social_lm)

# homoschedasticity
boxplot(res ~ df$CNT,las=2)

pdf("residual_plot.pdf")
plot(fitted(fit_social_lm), res, col = df$CNT)
dev.off
```
