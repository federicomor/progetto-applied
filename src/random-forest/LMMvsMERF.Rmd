...
data <- read.csv(file="std_scores_data_wo_outliers.csv")
head(data)
```
...
to_discard <- c("CNT", "Social.well.being", "Psychological.well.being", "SCHLTYPE","SchoolUse.ICT")
...
#LMM
selected_regressors <- c("CompInt.ICT","Teacher.skills", "HEDRES","CULTPOSS","ATTLNACT","PROAT5AB","CLSIZE","CREACTIV","SCHSIZE","PV1READ","LM_MINS", "ENTUSE")

FORMULA_LMM <- paste(paste("Social.well.being","~"), paste(c(selected_regressors,"(1|CNT)"), collapse = "+"))
```
...
partialPlot(fit_lmerf$forest.model,
              pred.data = data,
              x.var = "HEDRES",
              main = "Dependence on students' perceived competence in using ICT"
            )
abline(a = 0, b = betas$HEDRES, col = "red")
dev.off()

pdf("fig/ATTLNACT.pdf")
partialPlot(fit_lmerf$forest.model,
              pred.data = data,
              x.var = "ATTLNACT",
              main = "Dependence on students' attitude towards learning"
            )
abline(a = 0, b = betas$ATTLNACT, col = "red")
dev.off()
...
partialPlot(fit_lmerf$forest.model,
              pred.data = data,
              x.var = "PV1READ",
              main = "Dependence on students' performance in reading"
            )
