...
soglia=2.41
# for(soglia in seq(1.5,6,by=0.05)){
# for(soglia in seq(a,b,by=delta)){
  ...
  print(paste("soglia=", soglia,"-> ",x))
  pvec = c(pvec,x)
  # print(paste("soglia=", soglia,"produce", round(na.omit(data)[1]/4876*100, digits=2)))
# }
dim(data)
data=na.omit(data)
dim(na.omit(data))
...
# abline(h=svec[19])
min(pvec[1:length(pvec)-1]-pvec[2:length(pvec)])
pvec[1:length(pvec)-1]-pvec[2:length(pvec)]
pvec[19]-pvec[20]
svec[19]
