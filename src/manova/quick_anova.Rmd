...
treat = levels(Glabel)
g = length(treat)
RADAR_MATRIX = data.frame(matrix(0,16,length(model_variables)))
...
```{r}
for(j in 1:length(model_variables)){
	RADAR_MATRIX[3:16,j] = RADAR_MATRIX[3:16,j] + abs(min(RADAR_MATRIX[3:16,j]))
	RADAR_MATRIX[1,j] = max(RADAR_MATRIX[3:16,j])
	RADAR_MATRIX[2,j] = min(RADAR_MATRIX[3:16,j])
}
RADAR_MATRIX
```
...
group_ordered <- with(df_social,                      
	...
	max2 = max(setdiff(differenze_discrete,max1))
		...
		plot(1:14,medie,type="b",main=scelta,ylab="medie(black) & ddif(blue)")
		points(1:14,medie,pch=19,main=scelta)
		lines(1:13,differenze_discrete,type="b",col="blue")
...
# for (i in 1:4 ){
	...
	boxplot(df_social[,model_variables[i]] ~group_ordered,
			...
			ylim=c(round(min(df_social[,model_variables[i]])),
				   round(max(df_social[,model_variables[i]]))),
			col = rev(colora(length(levels(Glabel)),20)))
	abline(h=mean(df_social[,model_variables[i]]))
