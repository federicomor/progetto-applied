...
for(k in 1:p){
    ...
    gruoup_ordered_list[[colnames(measures)[k]]] = data.frame(levels(group_ordered),livelli_fittizzi)
    gruoup_ordered_list[[colnames(measures)[k]]]$livelli_fittizzi=as.factor( gruoup_ordered_list[[colnames(measures)[k]]]$livelli_fittizzi)
...
for (k in 1:p){
    ...
    print(
      ...
      geom_sf(size = 0.2, color = "black") + # border line
    scale_fill_manual(
    values = c("low" = "royalblue", "medium" = "forestgreen", "high" = "darkorange"),
    na.value = "white"  # Specify the color for NA values
  ) +
      labs(
        title =colnames(measures)[k],
