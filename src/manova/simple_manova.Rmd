...
for(j in 1:p){
  group_ordered <- with(data,                       # Order boxes by median
                      reorder(data$label,
                              data.feats[,j],
                              mean))
  boxplot(data.feats[,j]~group_ordered, main=paste(colnames(data.feats)[j]),las=2,
          ylim=c(round(min(data.feats[,j])),
          round(max(data.feats[,j]))), col = rainbow(g))
   abline(h=0)
}
#### ADAPT YLIM
ylim = 15
### cambia nome titolo
for(k in 1:p){
  plot(c(1,g*(g-1)/2), xlim=c(c(1,g*(g-1)/2)), pch='',ylim=c(-2,5),
       xlab='pairs treat', ylab=paste('CI tau',colnames(data.feats)[k]), main=paste('CI tau',colnames(data.feats)[k]))
  
 for(i in 1:(g-1)) {
  for(j in (i+1):g) {
    ind <- (i-1)*g-i*(i-1)/2+(j-i)
    
    lines (c(ind,ind), c(CI[[ind]][k,1],CI[[ind]][k,2])); 
    points(ind, mg[i,k]-mg[j,k], pch=16); 
    points(ind, CI[[ind]][k,1], col=rainbow(g)[j], pch=16); 
    points(ind, CI[[ind]][k,2], col=rainbow(g)[i], pch=16); 
   }
  }
  abline(h=0)
}
