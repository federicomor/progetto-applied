...
for(k in 1:p){
  ...
  abline(h=0)
  plot(c(1,g*(g-1)/2), xlim=c(c(1,g*(g-1)/2)), pch='',ylim=c(min(unlist(CI)),max(unlist(CI))),
 ...
 for(i in 1:(g-1)) {
   flag=0
     for(j in (i+1):g) {
        ind <- (i-1)*g-i*(i-1)/2+(j-i)
        ii = which( levels(group_ordered)==treat[i])
        jj = which( levels(group_ordered)==treat[j])
        ind2 <- (ii-1)*g-ii*(ii-1)/2+(jj-ii)
        if(CI[[ind2]][k,1]*CI[[ind2]][k,3]<0 && flag==0){
          flag=1
            lines (c(ind,ind), c(CI[[ind2]][k,1],CI[[ind2]][k,3])); 
            #points(ind, mg[ii,k]-mg[jj,k], pch=16); 
            points(ind, CI[[ind2]][k,1], col=rainbow(g)[jj], pch=16); 
            points(ind, CI[[ind2]][k,2], col="black", pch=16); 
            points(ind, CI[[ind2]][k,3], col=rainbow(g)[ii], pch=16); 
        }
