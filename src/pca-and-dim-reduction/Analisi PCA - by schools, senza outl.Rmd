...
df=read.csv("../../data/pisa-woNA_school_final_wo_Outl_ClassicData.csv")

vec=df$SWB[which(df$CNT=="DNK")]
print(paste("DNK",length(vec)))
# DEN ne ha uno solo, lo togliamo
# aveva! aveva solo uno, il nuovo dataset ha sistemato questo
...
### Scores dataset
```{r}
df_sc = pca$scores[,1:5]
colnames(df_sc) = c("scPC1_tec","scPC2_tec","scPC3_tec","scPC4_tec","scPC5_tec")
```
...
```{r}
dim(df_sc)
names= c(colnames(df_sc),"scPC1_psi","scPC2_psi","scPC3_psi","scPC4_psi","scPC5_psi")
df_sc = cbind(df_sc, pca$scores[,1:5])
...
```{r}
dim(df_sc)
names= c(colnames(df_sc),"scPC1_clt","scPC2_clt","scPC3_clt")
df_sc = cbind(df_sc, pca$scores[,1:3])
colnames(df_sc) = names
```
...
```{r}
dim(df_sc)
names = c(colnames(df_sc),"scPC1_tch")
df_sc = cbind(df_sc, pca$scores[,1])
colnames(df_sc) = names
...
write.csv(df_sc_with_cnt,"../../data/pisa-woNA_school_final_wo_Outl_ClassicData_PCA_SCORES.csv")
```
