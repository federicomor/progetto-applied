...
mvn(data = select_if(pisa_data,is.numeric), mvnTest = "hz",univariateTest = "SW")
```
...
#eigenvalues method
quartz()
scree(select_if(pisa_data,is.numeric), pc=TRUE)
#parallel analysis
quartz()
...
```{r}
#maximum likelihoood method: assuming normality
...
#Principal Component method
for(i in 1:dim(fit_PC$loadings)[2]){
  barplot(fit_PC$loadings[,i], main = paste("factor",i),las=2,cex.names=0.7)
}
```
