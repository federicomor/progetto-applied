...
regfit.full = regsubsets(FORMULA_COMPLETE_SOCIAL, data=data)
...
##  K-fold-cross-validation (exhaustive search)
```{r, warning=FALSE, message=FALSE}
p = nv_max
k = 30
...
cv.errors = matrix(NA,k,p, dimnames=list(NULL, paste(1:nv_max)))

for(j in 1:k){
  best.fit = regsubsets(FORMULA_COMPLETE_SOCIAL,data=data[folds!=j,],nvmax=nv_max,really.big=T)
  for(i in 1:p){
...
reg.best = regsubsets(FORMULA_COMPLETE_SOCIAL.,data=data, nvmax=nv_max)
coef(reg.best,10)
...
## Best Subset Selection (exhaustive search) 
```{r, warning=FALSE, message=FALSE}
regfit.full = regsubsets(FORMULA_COMPLETE_PSICO, data=data)
# Best Subset Selection
summary(regfit.full)
