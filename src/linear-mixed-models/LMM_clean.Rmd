...
```{r}
vars = c(linear_model_vars,"IM_PUBLIC")
...
vars=c(linear_model_vars,"(1|NEW_VAR)")
...
for(i in 1:(n-2)){    #-2 per togliere CNT e NEW_VAR (già in NEW_VAR)
  slopevar=vars[i]
...
#ATTLNACT, JOYREAD, PV1MATH e PV1READ sembrano promettenti ma AIC molto vicini tra loro 
...
# Due slopes
```{r}
#Ciclo come prima, stavolta considero già una slope, in questo caso ATTLNACT (PV1READ), che sembrava dare il maggior miglioramento AIC
#in realtà 0.00...
...
#questo è esempio con PV1READ

vars2=c(vars,"(PV1READ|NEW_VAR)")
n=length(vars2)

i=1
while(i < (n-3)){    #-3 per togliere CNT, NEW_VAR e slope 
  slopevar=vars2[i]
  if(slopevar=="PV1READ"){
    i=i+1
    slopevar=vars2[i]
...
#forse si può aggiungere ATTLNACT
```


#nel caso di sia PV1READ sia ATTLNACT 
```{r}
vars3=c(vars,"(PV1READ|NEW_VAR)","(ATTLNACT|NEW_VAR)")
...
## visualization of the random intercepts with their 95% confidence intervals
# Random effects: b_0i for i=1,...,234
dotplot(ranef(fit_psico_lmm, condVar=T))$NEW_VAR
...
for(i in 1:(n-2)){    #-1 per togliere CNT e NEW_VAR (già in NEW_VAR)
  slopevar=vars[i]
  varsfor=c(vars,paste("(",paste(slopevar),"|NEW_VAR)",collapse=""))
...
# Non vale davvero la pena, miglioramento ancora più infimo di prima
# Due slopes
```{r}
