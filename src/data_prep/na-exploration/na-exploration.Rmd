...
pisa_data$SCHLTYPE <- as.factor(pisa_data$SCHLTYPE)
plot(pisa_data$SCHLTYPE)
```
# add SCHLTYPE
```{r}
pisa_schtype = pisa_data[,c(1,3,6)]
root_proj_dir = "../../../"
...
pisa_wo_schtype <- read.csv(file=dataset_dir)
head(pisa_wo_schtype)
merged_df <- merge(pisa_wo_schtype, pisa_schtype[, c("CNTSCHID", "SCHLTYPE")], by = "CNTSCHID", all.x = TRUE)

# Select the desired columns from the merged dataset
final_df <- merged_df[, c(colnames(pisa_wo_schtype), "SCHLTYPE")]

# Print and save the final dataset
head(final_df)
write.csv(final_df, "../../../data/pisa_wPV_grouped_bysch_schtype.csv", row.names = FALSE)
```
